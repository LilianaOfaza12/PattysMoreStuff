...
public class TileEntityIronFurnace extends TileEntityLockable implements ITickable, ISidedInventory {
    ...
    {
        return ConfigPreInit.ironFurnaceCookSpeed;
    }
    /**
    ...
    public static int getItemBurnTime(ItemStack stack)
    {
        ...
        else
        {
            ...
            if (item instanceof net.minecraft.item.ItemBlock && Block.getBlockFromItem(item) != Blocks.AIR)
            {
                ...
                {
                    return ConfigPreInit.ironFWoodenSlabBurnTime;
                }

                if (block.getDefaultState().getMaterial() == Material.WOOD)
                {
                    return ConfigPreInit.ironFWoodMaterialBurnTime;
                }

                if (block == Blocks.COAL_BLOCK)
                {
                    return ConfigPreInit.ironFCoalBlockBurnTime;
                }
            }

            if (item instanceof ItemTool && "WOOD".equals(((ItemTool)item).getToolMaterialName())) return ConfigPreInit.ironFToolWoodBurnTime;
            if (item instanceof ItemSword && "WOOD".equals(((ItemSword)item).getToolMaterialName())) return ConfigPreInit.ironFSwordWoodBurnTime;
            if (item instanceof ItemHoe && "WOOD".equals(((ItemHoe)item).getMaterialName())) return ConfigPreInit.ironFHoeWoodBurnTime;
            if (item == Items.STICK) return ConfigPreInit.ironFStickBurnTime;
            if (item == Items.COAL) return ConfigPreInit.ironFCoalBurnTime;
            if (item == Items.LAVA_BUCKET) return ConfigPreInit.ironFLavaBucketBurnTime;
            if (item == Item.getItemFromBlock(Blocks.SAPLING)) return ConfigPreInit.ironFSaplingBurnTime;
            if (item == Items.BLAZE_ROD) return ConfigPreInit.ironFBlazeRodBurnTime;
