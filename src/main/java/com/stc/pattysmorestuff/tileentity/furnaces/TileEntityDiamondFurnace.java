...
public class TileEntityDiamondFurnace extends TileEntityLockable implements ITickable, ISidedInventory {
    ...
    {
        return ConfigPreInit.diamondFurnaceCookSpeed;
    }
    ...
    public static int getItemBurnTime(ItemStack stack)
    {
        ...
        else
        {
            ...
            if (item instanceof net.minecraft.item.ItemBlock && Block.getBlockFromItem(item) != Blocks.AIR)
            {
                ...
                {
                    return ConfigPreInit.diamondFWoodenSlabBurnTime;
                }

                if (block.getDefaultState().getMaterial() == Material.WOOD)
                {
                    return ConfigPreInit.diamondFWoodMaterialBurnTime;
                }

                if (block == Blocks.COAL_BLOCK)
                {
                    return ConfigPreInit.diamondFCoalBlockBurnTime;
                }
            }

            if (item instanceof ItemTool && "WOOD".equals(((ItemTool)item).getToolMaterialName())) return ConfigPreInit.diamondFToolWoodBurnTime;
            if (item instanceof ItemSword && "WOOD".equals(((ItemSword)item).getToolMaterialName())) return ConfigPreInit.diamondFSwordWoodBurnTime;
            if (item instanceof ItemHoe && "WOOD".equals(((ItemHoe)item).getMaterialName())) return ConfigPreInit.diamondFHoeWoodBurnTime;
            if (item == Items.STICK) return ConfigPreInit.diamondFStickBurnTime;
            if (item == Items.COAL) return ConfigPreInit.diamondFCoalBurnTime;
            if (item == Items.LAVA_BUCKET) return ConfigPreInit.diamondFLavaBucketBurnTime;
            if (item == Item.getItemFromBlock(Blocks.SAPLING)) return ConfigPreInit.diamondFSaplingBurnTime;
            if (item == Items.BLAZE_ROD) return ConfigPreInit.diamondFBlazeRodBurnTime;
